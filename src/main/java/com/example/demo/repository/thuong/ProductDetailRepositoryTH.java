...
public interface ProductDetailRepositoryTH extends JpaRepository<ProductDetail, Long> {
    ...
    @Query(value = "SELECT pd FROM ProductDetail pd JOIN pd.product p JOIN pd.color c WHERE pd.status = 1 AND  pd.product.status = 1 AND (p.name LIKE %:keyword% OR c.name LIKE %:keyword% OR p.code LIKE %:keyword% OR CONCAT(p.name, ' ', c.name) LIKE %:keyword%)")
